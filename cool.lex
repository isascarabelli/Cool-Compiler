...
<STRING> \" { // handles the termination of a string constant when a closing quote is encountered. If an escape character appears before a quote, we must put a quote into the string
  if(stringTooLong) {
    yybegin(YYINITIAL);
